...
class PointsRepository extends \Doctrine\ORM\EntityRepository
{
    ...
    // TODO zmienić na querybuiklder jak wystarczy czasu !!
    public function findByDistanceResult($userLatitude, $userLongitude, $distance){
        $manager = $this->getEntityManager();
        $connection = $manager->getConnection();
        $query = $connection->prepare("SELECT poi.points.id, poi.points.longitude, poi.points.latitude, poi.points.rating, poi.points.name, poi.points.locality, poi.points.description, poi.points.picture, poi.points.mimetype, DATE_FORMAT(poi.points.AddedDate, '%Y-%m-%d') AS addeddate, poi.points.Type_Id AS typeid, poi.points.User_Id AS userid, ROUND((6378.137*1000 * ACos( Cos( RADIANS(poi.points.Latitude) ) * Cos( RADIANS( :userLatitude ) ) * Cos( RADIANS( :userLongitude ) - RADIANS(poi.points.Longitude) ) + Sin( RADIANS(poi.points.Latitude) ) * Sin( RADIANS( :userLatitude ) ) )), 0) AS distance FROM poi.points HAVING distance <= :distance;");
